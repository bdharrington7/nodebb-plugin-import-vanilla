...
Export.prototype = {
		...
		this.c.query(query,
			function(err, rows){
				...
				_self._normalizeCategories(rows, function(_cids){
					...
					_self.logger.info('now writing categories array to disk, please be patiend');
		...
		this.c.query(query,
			function(err, rows) {
				...
				_self._normalizeTopics(rows, function(_tids){
					...
					_self.logger.info('now writing topics array to disk, please be patiend');
	...
	exportPosts: function (next) {
		...
		this.c.query(query, function(err, rows) {
			...
			_self._normalizePosts(rows, function(_pids){
				...
				_self.logger.info('now writing posts array to disk, please be patiend');
		...
		async.eachLimit(rows, 5, function(row, done) {
			...
			if (storedPost.normalized || storedPost.skipped) {
				...
				if (normalizedTopic && normalizedUser) {
					...
					var requiredValues = [normalizedTopic, normalizedUser];
					var requiredKeys = ['normalizedTopic', 'normalizedUser'];
					var falsyIndex = _self._whichIsFalsy(requiredValues);
	...
	report: function(next) {
		...
		logger.raw('\n\n====  REMEMBER:\n'
			+ '\n\t*-) Email all your users their new passwords, a'
